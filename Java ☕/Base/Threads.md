...
- 
	...
	- void join(): Obliga a un hilo a ejecutarse hasta el final antes que empieze otro( recomendable que sea el primer hilo en ejecutarse), va antes del 'start' de cualquier otro hilo
```Java
...
- Nos permite sincronizar (muchos)hilos en programas complejos
- Estos hilos intentan ejecutar simultaneamente un mismo metodo. Para sincronizarlos todos es necesario usar esta clase

- **Constructor:**
	- Reentrantlock(): Basico
...
```Java
public void metodo(){
	Lock bloquearHilo = new ReentrantLock();
	bloquearHilo.lock();
	try{
		codigo
	} finally {
...
- Siempre va dentro de un *while* que evalua la condicion
...
```Java
Condition evaluarCantidad = new bloquearHilo.newCondition();

public void bloqueConCondicion(){
	bloquearHilo.lock();
	try{
		while(n<20){
		evaluarCantidad.await();     // Miestras n<20, este hilo quedara en espera hasta que salga del bucle, la cual se encargaran los otros hilos que vayan entrando
		}
		codigo;
		evaluarCantidad.signalAll();
	} finally {
		bloquearHilo.unlock()'
	}
}
...
## SYNCHRONIZED
- Nos permite agregarla a metodos que seran usados por diferentes hilos para que no se solapen entre si y solo pueda ser usado por un hilo a la vez. Crea un bloqueo implicito
	- Similar al bloqueo explicito de la clase ReentrantLock
- Es lo mismo que usar un 'join' lo que vendria a reemplazarlo
- Tambien se puede usar como bloqueo que recibe un parametro 
- No es necesario los metodos lock y unlock, son reemplazados por wait y notifyAll

---

## [**Class Object**](https://docs.oracle.com/javase/8/docs/api/java/lang/Object.html "class in java.lang")
